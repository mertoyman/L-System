...
#include "Curves/curve_app.h"
#include "Fluids/fluids_app.h"
...
namespace octet {
  static app *app_factory(const char *name, int argc, char **argv) {
    ...
    else if (!strcmp(name, "Lsystem")) return new LSystem(argc, argv); //  L Systems
	  else if (!strcmp(name, "curves")) return new Curve_app(argc, argv); // 
    else if (!strcmp(name, "fluids")) return new fluids_app(argc, argv);
	  else return 0;
  }
