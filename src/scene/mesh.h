...
namespace octet {
  ...
  public:
    ...
    void enable_attribute_Color() const {
      unsigned n = normalized;
        unsigned size = get_size(2);
        unsigned kind = get_kind(2);
        unsigned attr = get_attr(2);
        unsigned offset = get_offset(2);
        glVertexAttribPointer(attribute_normal, size, kind, n & 1, get_stride(), (void*)(offset));
        glEnableVertexAttribArray(attr);
        n >>= 1;
    }
