...
namespace octet {
  template <class item_t, class allocator_t=allocator, bool use_new_delete=true> class dynarray {
    ...
    void _copy(const dynarray& rhs) {
      ...
      int_size_t insertIndex = 0;
      for (int_size_t readIndex = 0; readIndex < rhs.size(); ++readIndex, ++insertIndex) {
        (*this)[insertIndex] = rhs[readIndex];
      }
    }
  ...
  public:
    ...
    public:
      ...
      item_t *operator ->() const { return &(*vec)[elem]; }
      item_t &operator *() const { return (*vec)[elem]; }
    ...
    void swap(dynarray& rhs) {
      octet::swap(data_, rhs.data_);
      octet::swap(size_, rhs.size_);
      octet::swap(capacity_, rhs.capacity_);
    };
  };
