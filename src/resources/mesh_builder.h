...
namespace octet {
  class mesh_builder {
    ...
    void add_front_face(float size, vec4 &color = vec4(1.0f,1.0,1.0f,1.0f) ) {
      unsigned short cur_vertex = (unsigned short)vertices.size();
      add_vertex(vec4(-size, -size, size, 1), vec4( color.x(), color.x(), color.x(), 1), 0, 0);
      add_vertex(vec4(-size,  size, size, 1), vec4( color.y(), color.y(), color.y(), 1), 0, 1);
      add_vertex(vec4( size,  size, size, 1), vec4( color.z(), color.z(), color.z(), 1), 1, 1);
      add_vertex(vec4( size, -size, size, 1), vec4( color.w(), color.w(), color.w(), 1), 1, 0);
  ...
  public:
    ...
    unsigned add_vertex(const vec4 &pos, const vec4 &normal, float u, float v) {
      ...
      //vec4 tnormal = normal * matrix; // matrix doesnt have the correct value for color!
      vec4 tnormal = normal;
    ...
    void change_color(const vec4 &color) {
      for ( unsigned int i = 0; i <= 5; i++) { // for each of 6 faces
        for ( unsigned int j = 0; j <= 4; j++) { // for each vertex
        vertices[j].normal[0] = color[j];
        vertices[j].normal[1] = color[j];
        vertices[j].normal[2] = color[j];
        }
      }
    }
    ...
    void add_cube_coordinates(float x, float y, float z, float size, vec4 &color) {
      matrix.loadIdentity(); 
      matrix.translate(x,y,z);

      add_front_face(size, color);     
      matrix.rotateY90();
      add_front_face(size, color);
      matrix.rotateY90();
      add_front_face(size, color);
